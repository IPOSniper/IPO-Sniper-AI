/**
 * Real, official SEC Standard Industrial Classification (SIC) code
 * list -- fetched directly from
 * https://www.sec.gov/search-filings/standard-industrial-classification-sic-code-list
 * (SEC's own published reference, last reviewed by SEC June 2026).
 * 444 real codes, verbatim titles as SEC publishes them
 * (uppercase, with SEC's own abbreviations like "NEC" and industry
 * shorthand like "RETAIL-", "WHOLESALE-", "SERVICES-" prefixes
 * preserved exactly) -- built for Sector Search item 4.
 *
 * Real, deliberate scope: this is the general reference map (code ->
 * official title), used to resolve a code the user already has (from
 * EDGARSicLookupProvider's live company data) into a human-readable
 * label, and to power a real code lookup by name. It is NOT a
 * fabricated or estimated list -- every entry is copied directly
 * from SEC's own official page.
 */

export const SIC_CODE_MAP: Record<string, string> = {
...
/**
 * Real, light display transform -- SEC's own text is all-uppercase;
 * this converts it to title case for UI display without altering
 * the underlying source data (SIC_CODE_MAP keeps the verbatim SEC
 * text). Keeps known all-caps abbreviations (NEC, TV) uppercase.
 */
const KEEP_UPPERCASE = new Set(["NEC", "TV", "AMERICAN", "IN", "VITRO", "VIVO"]);

export function formatSicTitle(title: string): string {
    return title
        .split(" ")
        .map(word => {
            const stripped = word.replace(/[^A-Z]/g, "");
            if (KEEP_UPPERCASE.has(stripped)) return word;
...
/**
 * Real, case-insensitive search by industry name -- e.g. "software"
