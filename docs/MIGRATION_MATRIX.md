...
| Workstation mission-control structural redesign (AI committee avatar grid, evidence terminal, sector performance widgets, institutional flow chart, breadcrumb, live header pill inside the shell) | Infrastructure branch (header pill) / `workstation_merged_navy.html` (the rest) | Core | Core | **Deferred, not Pending** | Explicitly NOT attempted this pass. This touches `WorkstationShell.tsx`'s actual panel registry (112 files: `ResearchViewport`, `IntelligenceSidebar`, and everything they render) — a previous session already flagged this exact risk directly: a blind restyle here could silently break real, working functionality, and I have no way to render/verify the result from this sandbox. Real next step: map `workstation_merged_navy.html`'s sections one at a time against the actual panel registry (which real panel does "AI Verdict" correspond to, which does "Institutional Flow," etc.) before changing anything — a dedicated pass, not a fast one. |
