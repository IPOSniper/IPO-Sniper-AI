...
| Workstation mission-control structural redesign (AI committee avatar grid, evidence terminal, sector performance widgets, institutional flow chart, breadcrumb, live header pill inside the shell) | Infrastructure branch (header pill) / `workstation_merged_navy.html` (the rest) | Core | Core | **Partial — Group A items landing** | `AIVerdictRow`, `CommitteeAvatarRow` (with real photo avatars + click-through workspace), and `MissionControlHeader` all real and wired. `AnalystWorkspace.tsx` is new — a real per-analyst deep-dive (evidence/assumptions/risks/unknowns/monitoring, none previously displayed anywhere) opened by clicking an avatar. Mission Control header deliberately omits "Next Refresh" countdown and "Live Market" — no real continuous refresh loop exists. Sector performance, institutional flow, evidence terminal still not attempted. Separately: user proposed a "living workspace" vision (analysts animating in real-time, evidence flowing, vote evolving) — sorted into 3 groups (real-now / needs-real-streaming-architecture / would-be-fabricated-regardless), documented in chat, not yet in this file. |
