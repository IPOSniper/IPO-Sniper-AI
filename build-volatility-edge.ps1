...
$path = "engine\quant\decisionMatrix\volatilityEdge.ts"
...
import { AlpacaBarsProvider } from "@/engine/trading/providers/AlpacaBarsProvider";
...
export async function detectVolatilityEdge(ticker: string): Promise<VolatilityEdgeResult> {
    try {
        const bars = await new AlpacaBarsProvider().getBars(ticker, "1Day", 22);
...
Write-Host "PATCHED: volatilityEdge.ts created." -ForegroundColor Green

Write-Host "`n=== VERIFY ===" -ForegroundColor Cyan
Get-Item "engine\quant\decisionMatrix\volatilityEdge.ts" | Select-Object Name, Length
...
Write-Host "`n=== AUDIT: real AlpacaBarsProvider signature ===" -ForegroundColor Cyan
Get-ChildItem -Recurse -File -Include "AlpacaBarsProvider.ts" | ForEach-Object {
    Get-Content $_.FullName | Select-String -Pattern "class AlpacaBarsProvider|getBars" -Context 0,5
}
